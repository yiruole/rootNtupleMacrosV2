...
void analysisClass::Loop() {
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    else // using the turn-on in the MC
    {
      ...
      // only look at lead electron, which should be ele1, but explicitly check this to be sure
      if(Ele1_PtHeep > Ele2_PtHeep)
      {
        pass_trigger = trigEle27::passTrig(Ele1_PtHeep,Ele1_SCEta) ? 1 : 0;
        trigEff = trigEle27::turnOn(Ele1_PtHeep,Ele1_SCEta);
      }
      else
      {
        pass_trigger = trigEle27::passTrig(Ele2_PtHeep,Ele2_SCEta) ? 1 : 0;
        trigEff = trigEle27::turnOn(Ele2_PtHeep,Ele2_SCEta);
      }
    }
