...
#
from ROOT import gROOT, TCanvas, TFile, TGraphAsymmErrors, TMultiGraph, TLegend, kMagenta
import os
import copy
...
def MakeTrigEffGraphs(inputFilesTrigs, IsEEJJ):
    ...
    for fileName in inputFilesTrigs:
        ...
        denominator = (
            tfile.Get("NEventsTwoGenEleECAL")
            if IsEEJJ
            else tfile.Get("NEventsOneGenEleECAL")
        )
        numerator = (
            tfile.Get("NEventsTwoGenEleECALPlusTrig")
            if IsEEJJ
            else tfile.Get("NEventsOneGenEleECALPlusTrig")
        )
...
def MakeIDEffGraphs(inputFilesIDs, IsEEJJ):
    ...
    for i, fileName in enumerate(inputFilesIDs):
        ...
        numerator = (
            tfile.Get("NEventsTwoGenEleECALPlusTrigPlusTwoID")
            if IsEEJJ
            else tfile.Get("NEventsOneGenEleECALPlusTrigPlusOneID")
        )
        numeratorTrigAndID = (
            tfile.Get("NEventsTwoGenEleECALPlusTrigPlusTwoID")
            if IsEEJJ
...
def MakeTrigAndIDEffMultigraph(
    trigger, trigNames, idNames, trigAndIdEffGraphsOrig, IsEEJJ
):
...
def MakeTrigAndIDEffMultigraphWithID(
    idname, trigNames, idNames, trigAndIdEffGraphsOrig, IsEEJJ
):
    # work with a copy, or else there are seg faults later
    trigAndIdEffGraphs = copy.deepcopy(trigAndIdEffGraphsOrig)
    mg3 = TMultiGraph()
    ...
    imageName = (
        "trigAndIdEff_"
        + idname.replace(" ", "_")
        + ("_eejj" if IsEEJJ else "_enujj")
        + ".png"
    )
    t7.Print(imageName)


def MakeFirstEleGenPtMultigraph(
    trigger, trigNames, idNames, idEffVsPt1stGenEleGraphs, IsEEJJ
):
    mg4 = TMultiGraph()
    ...
    leg4.Draw()
    imageName = (
        "idEffVs1stEleGenPt_" + trigger + ("_eejj" if IsEEJJ else "_enujj") + ".png"
    )
    t8.Print(imageName)
...
enujjInputFilesTrigs.append(
    enujjAnalysisDir
    + "output_cutTable_lq1_eejj_trigger4_id1_effiStudy/combined_enujj.root"
)
enujjInputFilesTrigs.append(
    enujjAnalysisDir
    + "output_cutTable_lq1_eejj_trigger5_id1_effiStudy/combined_enujj.root"
)
enujjInputFilesTrigs.append(
    enujjAnalysisDir
    + "output_cutTable_lq1_eejj_trigger4_id0_effiStudy/combined_enujj.root"
)
enujjInputFilesTrigs.append(
    enujjAnalysisDir
    + "output_cutTable_lq1_eejj_trigger5_id0_effiStudy/combined_enujj.root"
)
enujjInputFilesTrigs.append(
    eejjAnalysisDir
    + "output_cutTable_lq1_eejj_trigger6_id0_effiStudy/combined_enujj.root"
)
enujjInputFilesTrigs.append(
    eejjAnalysisDir
    + "output_cutTable_lq1_eejj_trigger7_id0_effiStudy/combined_enujj.root"
)
