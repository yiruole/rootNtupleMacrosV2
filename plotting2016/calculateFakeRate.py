...
def getNumeratorHist   (root_file, variable, ptMin, ptMax, rebin ) :
    ...
    jets_hist1D = jets_hist2D.ProjectionY("Jets_"+variable+"_1D",jets_hist2D.GetXaxis().FindBin(ptMin),jets_hist2D.GetXaxis().FindBin(ptMax))
...
def getDenominatorHist (root_file, variable, ptMin, ptMax, rebin ) :
    ...
    total_hist1D = total_hist2D.ProjectionY("Total_"+variable+"_1D",total_hist2D.GetXaxis().FindBin(ptMin),total_hist2D.GetXaxis().FindBin(ptMax))
...
def getElectronHist (root_file, variable, ptMin, ptMax, rebin ) :
    ...
    total_hist1D = total_hist2D.ProjectionY("Electrons_"+variable+"_1D",total_hist2D.GetXaxis().FindBin(ptMin),total_hist2D.GetXaxis().FindBin(ptMax))
    hist = total_hist1D.Clone()
    return hist
...
def makePlots ( psfile, canvas, root_file, variable, xtitle, xmin, xmax, ymin, ymax, rebin ) :
    ...
    ptBins = [35,45,55,65,75,85,95,105,125,145,165,185,205,250,300,400,500,600,700,800,900,1000]
    ptBinsArr = array.array('f',ptBins)
    #ratio = TGraphErrors()
    ratio = ROOT.TH1F('fakeRate','',len(ptBins)-1,ptBinsArr)
    for index,pt in enumerate(ptBins):
      if index >= len(ptBins)-1: break
      ...
      jets_SR = n_hist.IntegralAndError(n_hist.FindBin(0),n_hist.FindBin(5),jets_SR_err)
      jets_SB = n_hist.IntegralAndError(n_hist.FindBin(10),n_hist.FindBin(20),jets_SB_err)
      ratioJets_SRSB = jets_SR/jets_SB
      ratioJets_SRSB_err = ratioJets_SRSB*math.sqrt(pow(jets_SB_err/jets_SB,2)+pow(jets_SR_err/jets_SR,2))
      # multiply by HEEP electrons in SB
      nHEEP_SB_err = ROOT.Double(0)
      nHEEP_SB = e_hist.IntegralAndError(n_hist.FindBin(10),n_hist.FindBin(20),nHEEP_SB_err)
      nJets_sig = ratioJets_SRSB*nHEEP_SB
      nJets_sig_err = nJets_sig*math.sqrt(pow(ratioJets_SRSB_err/ratioJets_SRSB,2)+pow(nHEEP_SB_err/nHEEP_SB,2))
      ...
      fake_rate_err = fake_rate*math.sqrt(pow(nJets_sig_err/nJets_sig,2)+pow(d_int_err/d_int,2))
...
makePlots ( plot_file_name, canvas, root_file, "Bar_TrkIsoHEEP7vsPt_PAS" , "1st Ele p_{T} (Barrel) [GeV]"              , 35, 500, 0.0, 0.03, 10 )
