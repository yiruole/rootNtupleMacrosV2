...
void analysisClass::Loop(){
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    sprintf(ele_jet_trigger_name     , "HLT_Ele30_CaloIdVT_TrkIdT_PFJet100_PFJet25_v%d"        , ele_jet_trigger_version     );	
    sprintf(ele_trigger_name         , "HLT_Ele30_CaloIdVT_TrkIdT_v%d"                         , ele_trigger_version         );
    ...
    // CollectionPtr trigger_25jet_all  = helper.GetHLTFilterObjects ( "hltEle30CaloIdVTTrkIdTDiCentralPFNoPUJet25EleCleaned"  );
    // CollectionPtr trigger_100jet_all = helper.GetHLTFilterObjects ( "hltEle30CaloIdVTTrkIdTDiCentralPFNoPUJet100EleCleaned" );
    CollectionPtr trigger_25jet_all  = helper.GetHLTFilterObjects ( "hltEle30CaloIdVTTrkIdTDiCentralPFJet25EleCleaned"  );
    CollectionPtr trigger_100jet_all = helper.GetHLTFilterObjects ( "hltEle30CaloIdVTTrkIdTDiCentralPFJet100EleCleaned" );
    HLTFilterObject leadHLTJet = trigger_25jet_all -> GetConstituent<HLTFilterObject>(0);

    int n_trigger_25jet_all  = trigger_25jet_all  -> GetSize();
    int n_trigger_100jet_all = trigger_100jet_all -> GetSize();
    ...
    CollectionPtr ele_HEEP = ele_all -> SkimByID<Electron>( HEEP );
    ...
    CollectionPtr pfjet_hltJet25Matched = pfjet_all -> SkimByRequireDRMatch<PFJet,HLTFilterObject>(trigger_25jet_all, 0.5);
    PFJet pfjet_matchedToLeadHLTJet = pfjet_hltJet25Matched -> GetClosestInDR<PFJet> ( leadHLTJet );

    //-----------------------------------------------------------------
    // Make turn-on curve
    //-----------------------------------------------------------------
    
    if ( ele_trigger_fired && n_trigger_25jet_all >= 2 && ele_jet_trigger_enabled){
    ...
    //   trigger_25jet_all      -> examine<HLTFilterObject>("HLT Jet 25");
    //   pfjet_hltJet25Matched  -> examine<PFJet>          ("PFJets, matched to 25 GeV HLT jets");
