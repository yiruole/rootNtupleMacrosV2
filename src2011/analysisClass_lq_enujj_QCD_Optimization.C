...
void analysisClass::Loop()
{
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
     ...
     if ( nJetLooseEle_Ana > 0 && nEle_QCDFake_Ana > 0 ) {
     ...
     if ( nEle_QCDFake > 0 && nJetLooseEle_Ana > 0 ) { 
     ...
     if ( nEle_QCDFake > 0 && nJetLooseEle_Ana > 1 ) { 
     ...
     if ( nEle_QCDFake > 0 ){
       if ( QCDFakeEle1_Pt > MET_Pt ) min_ElePt_MET = MET_Pt ;
       else                           min_ElePt_MET = QCDFakeEle1_Pt;
     }

     if ( nEle_QCDFake > 0 && nJetLooseEle_Ana > 1){
       fillVariableWithValue("MTenu_final"    ,  MT_Ele1MET              , min_prescale * fakeRate );
       					      
       fillVariableWithValue("ST_opt"         ,  sT_enujj                , min_prescale * fakeRate );
       fillVariableWithValue("Mej_min_opt"    ,  Mej                     , min_prescale * fakeRate );
       
       fillVariableWithValue("MTjnu_opt"      ,  MT_Jet1MET              , min_prescale * fakeRate );
       fillVariableWithValue("MinElePtMET_opt",  min_ElePt_MET           , min_prescale * fakeRate );
     }
