...
void analysisClass::Loop()
{
  ...
  double PDGID_GRAV = getPreCutValue1("PDGID_GRAV");  
  double PDGID_DSTAR = getPreCutValue1("PDGID_DSTAR");  
  double PDGID_USTAR = getPreCutValue1("PDGID_USTAR");  
  double PDGID_ZPRIME = getPreCutValue1("PDGID_ZPRIME");  
  double PDGID_GLUON = getPreCutValue1("PDGID_GLUON");  
  double PDGID_UP = getPreCutValue1("PDGID_UP");  
  double PDGID_DOWN = getPreCutValue1("PDGID_DOWN");  
  double PDGID_CHARM = getPreCutValue1("PDGID_CHARM");  
  double PDGID_STRANGE = getPreCutValue1("PDGID_STRANGE");  
  double PDGID_TOP = getPreCutValue1("PDGID_TOP");  
  double PDGID_BOTTOM = getPreCutValue1("PDGID_BOTTOM");  
  ...
  //for (Long64_t jentry=0; jentry<2000;jentry++) { // Begin of loop over events
	...
	for (int genp=0; genp<GenParticlePdgId->size(); genp++)
	  {
	    ...
	    //Search for specific final state
	    if( GenParticleMotherIndex->at(genp) != -1 )
	      {  
		if( 
		   ( 
		    abs( GenParticlePdgId->at( GenParticleMotherIndex->at(genp) ) ) == PDGID_GRAV
		    || abs( GenParticlePdgId->at( GenParticleMotherIndex->at(genp) ) ) == PDGID_DSTAR
		    || abs( GenParticlePdgId->at( GenParticleMotherIndex->at(genp) ) ) == PDGID_USTAR	
		    || abs( GenParticlePdgId->at( GenParticleMotherIndex->at(genp) ) ) == PDGID_ZPRIME
		    )
		   && GenParticleStatus->at(genp) == 3
		   )
		  { 
		    if( abs(GenParticlePdgId->at(genp)) == PDGID_GLUON )
		      {
			isGluon.push_back(1);
			//			cout << "found gluon" << endl;
		      }
		    
		    if( abs(GenParticlePdgId->at(genp)) == PDGID_UP 
			|| abs(GenParticlePdgId->at(genp)) == PDGID_DOWN  
			|| abs(GenParticlePdgId->at(genp)) == PDGID_CHARM  
			|| abs(GenParticlePdgId->at(genp)) == PDGID_STRANGE  
			|| abs(GenParticlePdgId->at(genp)) == PDGID_TOP  
			|| abs(GenParticlePdgId->at(genp)) == PDGID_BOTTOM  
			)
		      {
			isQuark.push_back(1);
	...
	CreateAndFillUserTH2D( "h2_Ngluon_vs_Nquark", 5, -0.5, 4.5, 5, -0.5, 4.5, isQuark.size(), isGluon.size() );
	if( isQuark.size() == 0 && isGluon.size() == 2 )
	  is_gg = 1;
	if( isQuark.size() == 1 && isGluon.size() == 1 )
	  is_qg = 1;
	if( isQuark.size() == 2 && isGluon.size() == 0 )
	  is_qq = 1;
    ...
    fillVariableWithValue( "is_gg", is_gg ) ;
    fillVariableWithValue( "is_qg", is_qg ) ;
    fillVariableWithValue( "is_qq", is_qq ) ;
