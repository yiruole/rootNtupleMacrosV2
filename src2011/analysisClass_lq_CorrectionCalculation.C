...
int getEtaBin ( double eta, int n_eta_bins, float * eta_bins ) { 
  
  for (int i = 0; i < n_eta_bins; ++i){
     float bin_min = eta_bins[i  ];
     float bin_max = eta_bins[i+1];
     if ( eta >= bin_min && eta < bin_max ) return i;
   }
...
void analysisClass::Loop()
{
   ...
   const int n_eta_bins = 6;
   float eta_bins[n_eta_bins + 1] = { -2.5, -1.5, -0.8, 0.0, 0.8, 1.5, 2.5 };
   int ncombos = 0;
   
   for (int i = 0; i < n_eta_bins; ++i){
     for (int j = i; j < n_eta_bins; ++j){
       ncombos++;
       char hist_name[100]; sprintf( hist_name, "Mee_EtaBin_%d_%d", i, j );
       CreateUserTH1D( hist_name, 60, 60, 120 ) ;
     } 
   }
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
     ...
     if ( passed_preselection ) {
       ...
       int eta_bin_1 = getEtaBin ( Ele1_Eta, n_eta_bins, eta_bins );
       int eta_bin_2 = getEtaBin ( Ele2_Eta, n_eta_bins, eta_bins );

       char hist_name[100]; sprintf( hist_name, "Mee_EtaBin_%d_%d", TMath::Min(eta_bin_1, eta_bin_2), TMath::Max(eta_bin_1,eta_bin_2));
       FillUserTH1D( hist_name, M_e1e2, pileup_weight * gen_weight );
