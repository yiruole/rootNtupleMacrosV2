...
bool isElectronBarrel(float superclusterEta) {
  return fabs(superclusterEta) < 1.479;
}
...
void analysisClass::Loop()
{
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    for(int i=0; i<c_genJetsNearEGLElectron->GetSize(); ++i) {
      ...
      if(lqMatchedJet)
        FillUserTH1D("genJetsNearEGLooseEle_matchedToLQ", 1, genWeight);
      else
        FillUserTH1D("genJetsNearEGLooseEle_matchedToLQ", 0, genWeight);
      for(int j=0; j<c_ele_egammaLoose->GetSize(); ++j) {
        ...
        if(dr < 0.3) {
          if(lqMatchedJet) {
            if(ele.PassHEEPID())
              FillUserTH1D("genJetsNearEGLooseEle_matchedToLQ_elePassHeep", 1, genWeight);
            else
              FillUserTH1D("genJetsNearEGLooseEle_matchedToLQ_elePassHeep", 0, genWeight);
          }
          else {
            if(ele.PassHEEPID())
              FillUserTH1D("genJetsNearEGLooseEle_unmatchedToLQ_elePassHeep", 1, genWeight);
            else
              FillUserTH1D("genJetsNearEGLooseEle_unmatchedToLQ_elePassHeep", 0, genWeight);
          }
        }
      }
    }
    FillUserTH1D("nGenJetsNearEGLooseEle",c_genJetsNearEGLElectron->GetSize(), genWeight);

    c_ele_all->examine<Electron>("c_ele_all");
    FillUserTH1D("nEleReco",c_ele_all->GetSize(), genWeight);
    FillUserTH1D("nEleReco_ptCut",c_ele_final_ptCut->GetSize(), genWeight);
    FillUserTH1D("nEleHEEP",c_ele_HEEP->GetSize(), genWeight);
    ...
    for(int i=0; i<c_ele_final->GetSize(); ++i) {
      ...
      FillUserTH1D("SCEtaElectron", ele.SCEta(), genWeight);
      FillUserTH1D("SCEtElectron", ele.SCEt(), genWeight);
      FillUserTH1D(("SCEtElectron"+electronNumber).c_str(), ele.SCEt(), genWeight);
      if(isElectronBarrel(ele.SCEta())) {
        FillUserTH1D("SCEtBarrel", ele.SCEt(), genWeight);
      }
      else {
        //if(!passHeep) {
        //  std::cout << "nanoaod run: " << static_cast<unsigned int>(run) << " lumi: " << static_cast<unsigned int>(ls) << " event: " << static_cast<unsigned int>(event) << std::endl;
        //  std::cout << "nanoaod endcap electron: pt=" << ele.Pt() << " scET=" << ele.SCEt() <<" scEta=" << ele.SCEta() << " phi=" << ele.Phi() << " passHEEP=" << passHeep << std::endl;
        //}
        FillUserTH1D("SCEtEndcap", ele.SCEt(), genWeight);
      }
      if(passHeep) {
        FillUserTH1D("SCEtElectronPassHEEP", ele.SCEt(), genWeight);
        FillUserTH1D(("SCEtElectron"+electronNumber+"PassHEEP").c_str(), ele.SCEt(), genWeight);
        if(isElectronBarrel(ele.SCEta())) {
          FillUserTH1D("SCEtBarrelPassHEEP", ele.SCEt(), genWeight);
        }
        else {
          FillUserTH1D("SCEtEndcapPassHEEP", ele.SCEt(), genWeight);
        }
      }
      else {
        if(isElectronBarrel(ele.SCEta())) {
          FillUserTH1D("SCEtBarrelFailHEEP", ele.SCEt(), genWeight);
        }
        else {
          FillUserTH1D("SCEtEndcapFailHEEP", ele.SCEt(), genWeight);
        }
      }
      if(passLoose)  {
        FillUserTH1D("SCEtElectronPassLoose", ele.SCEt(), genWeight);
        FillUserTH1D(("SCEtElectron"+electronNumber+"PassLoose").c_str(), ele.SCEt(), genWeight);
        if(fabs(ele.SCEta()) < 1.4442) {
          FillUserTH1D("SCEtBarrelPassLoose", ele.SCEt(), genWeight);
        }
        else if((fabs(ele.SCEta()) > 1.566 && fabs(ele.SCEta()) < 2.5)) {
          FillUserTH1D("SCEtEndcapPassLoose", ele.SCEt(), genWeight);
        }
      }
      else {
        if(isElectronBarrel(ele.SCEta())) {
    ...
    if( c_ele_all->GetSize() >= 1) {
      ...
      if ( c_ele_all->GetSize() >= 2 ) {
        ...
        if(ele1PassHeep && ele2PassHeep)
          FillUserTH1D("heepElectronsCategory", 1, genWeight);
        if ( c_ele_all->GetSize() >= 3 ) {
          ...
          if(ele2PassHeep && ele3PassHeep)
            FillUserTH1D("heepElectronsCategory", 2, genWeight);
          if(ele1PassHeep && ele3PassHeep)
            FillUserTH1D("heepElectronsCategory", 3, genWeight);
        }
      }
    }
    ...
    CollectionPtr c_muon_eta_IDHighPt = c_muon_all;
    //CollectionPtr c_muon_eta_IDLoose       = c_muon_eta       -> SkimByID      <Muon> ( MUON_LOOSE);
    CollectionPtr c_muon_eta_IDLoose= c_muon_all;
    ...
    CollectionPtr c_muon_final_ptCut       = c_muon_final;
    ...
    // XXX muons removed
    CollectionPtr c_pfjet_central_ID_noLeptonOverlap  = c_pfjet_central_ID     -> SkimByVetoDRMatch<PFJet, Electron>( c_ele_egammaLoose    , jet_ele_DeltaRCut );
    ...
    CollectionPtr c_pfjet_central_ID_eleOverlap  = c_pfjet_central_ID     -> SkimByRequireDRMatch<PFJet, Electron>( c_ele_egammaLoose    , jet_ele_DeltaRCut );
    c_pfjet_central_ID_eleOverlap->examine<PFJet>("c_pfjet_central_ID_eleOverlap (PFJets near loose ele)");
    CollectionPtr c_pfjet_central_ID_genEleLQOverlap = c_pfjet_central_ID->SkimByRequireDRMatch<PFJet, GenParticle>( c_genEle_fromLQ_finalState, jet_ele_DeltaRCut);
      ...
      for(int j=0; j<c_ele_egammaLoose->GetSize(); ++j) {
        ...
        if(dr < 0.1) {
          if(!ele.PassHEEPID()) {
            ...
            FillUserTH1D("DeltaRMatchJetEle_eleFailHeep", dr, genWeight);
            FillUserTH1D("DeltaPtMatchJetEle_eleFailHeep", deltaPt, genWeight);
            FillUserTH2D("DeltaRVsDeltaPtMatchJetEle_eleFailHeep", deltaPt, dr, genWeight);
            ...
            FillUserTH1D("DeltaRMatchJetEle_elePassHeep", dr, genWeight);
            FillUserTH1D("DeltaPtMatchJetEle_elePassHeep", deltaPt, genWeight);
            FillUserTH2D("DeltaRVsDeltaPtMatchJetEle_elePassHeep", deltaPt, dr, genWeight);
          }
        }
      }
    }
    ...
    FillUserTH1D("nJetFinalMatchedLQ",c_pfJetMatchedLQ->GetSize(), genWeight);
    FillUserTH1D("nJetFinal",c_pfjet_final->GetSize(), genWeight);
    for(int i=0; i<c_pfjet_final->GetSize(); ++i) {
      if(i > 5) continue;
      PFJet jet = c_pfjet_final -> GetConstituent<PFJet>(i);
      FillUserTH1D(("JetFinalPtJet"+std::to_string(i+1)).c_str(), jet.Pt(), genWeight);
    }
    for(int i=0; i<c_pfJetMatchedLQ->GetSize(); ++i) {
      PFJet jet = c_pfJetMatchedLQ -> GetConstituent<PFJet>(i);
      int idx = jet.GetRawIndex();
      if(idx >= 0)
        FillUserTH1D("idxJetFinalMatchedLQ", idx, genWeight);
    }
    //-----------------------------------------------------------------
    ...
    //XXX removed muons
    CollectionPtr c_pfjet_highEta_ID_noLeptonOverlap = c_pfjet_highEta_ID     -> SkimByVetoDRMatch<PFJet, Electron>( c_ele_final_ptCut    , jet_ele_DeltaRCut );
    ...
    if ( n_ele_store >= 1 ){
      ...
      if ( n_jet_store >= 1 ){ 
        ...
        FillUserTH1D("deltaREle1Jets", t_ele1.DeltaR ( t_jet1 ), genWeight);
        if(ele1LQMotherIdx == jetLQMotherIdx)
          FillUserTH1D("deltaR_Ele1JetSameLQ", t_ele1.DeltaR ( t_jet1 ), genWeight);
        if(ele1PassHeep) {
          FillUserTH1D("deltaREleJetsPassHEEP", t_ele1.DeltaR ( t_jet1 ), genWeight);
          if(isElectronBarrel(ele1.SCEta()))
            FillUserTH1D("deltaREleJetsBarrelPassHEEP", t_ele1.DeltaR ( t_jet1 ), genWeight);
          else
            FillUserTH1D("deltaREleJetsEndcapPassHEEP", t_ele1.DeltaR ( t_jet1 ), genWeight);
        }
        else {
          FillUserTH1D("deltaREleJetsFailHEEP", t_ele1.DeltaR ( t_jet1 ), genWeight);
        ...
        if ( n_jet_store >= 2 ){ 
          ...
          if(ele1LQMotherIdx == jetLQMotherIdx)
            FillUserTH1D("deltaR_Ele1JetSameLQ", t_ele1.DeltaR ( t_jet2 ), genWeight);
          FillUserTH1D("deltaREle1Jets", t_ele1.DeltaR ( t_jet2 ), genWeight);
          if(ele1PassHeep) {
            FillUserTH1D("deltaREleJetsPassHEEP", t_ele1.DeltaR ( t_jet2 ), genWeight);
            if(isElectronBarrel(ele1.SCEta()))
              FillUserTH1D("deltaREleJetsBarrelPassHEEP", t_ele1.DeltaR ( t_jet2 ), genWeight);
            else
              FillUserTH1D("deltaREleJetsEndcapPassHEEP", t_ele1.DeltaR ( t_jet2 ), genWeight);
          }
          else {
            FillUserTH1D("deltaREleJetsFailHEEP", t_ele1.DeltaR ( t_jet2 ), genWeight);
            //if(ele1.PassHEEPGsfEleSCEtaMultiRangeCut()) {
            //  if(isElectronBarrel(ele1.SCEta()))
          ...
          if ( n_jet_store >= 3 ){ 
            ...
            FillUserTH1D("deltaREle1Jets", t_ele1.DeltaR ( t_jet3 ), genWeight);
            if(ele1LQMotherIdx == jetLQMotherIdx)
              FillUserTH1D("deltaR_Ele1JetSameLQ", t_ele1.DeltaR ( t_jet3 ), genWeight);
            if ( n_jet_store >= 4 ){ 
              ++jetIdx;
              jetLQMotherIdx = getJetLQMotherIdx(c_genJet_all, c_pfjet_final, jetRawIndicesMap[jetIdx], c_genQuark_hardProcess);
              TLorentzVector t_ele1jet4 = t_ele1 + t_jet4;
              FillUserTH1D("deltaREle1Jets", t_ele1.DeltaR ( t_jet4 ), genWeight);
              if(ele1LQMotherIdx == jetLQMotherIdx)
                FillUserTH1D("deltaR_Ele1JetSameLQ", t_ele1.DeltaR ( t_jet4 ), genWeight);
              if ( n_jet_store >= 5 ){ 
                ++jetIdx;
                jetLQMotherIdx = getJetLQMotherIdx(c_genJet_all, c_pfjet_final, jetRawIndicesMap[jetIdx], c_genQuark_hardProcess);
                TLorentzVector t_ele1jet5 = t_ele1 + t_jet5;
                FillUserTH1D("deltaREle1Jets", t_ele1.DeltaR ( t_jet5 ), genWeight);
                if(ele1LQMotherIdx == jetLQMotherIdx)
                  FillUserTH1D("deltaR_Ele1JetSameLQ", t_ele1.DeltaR ( t_jet5 ), genWeight);
                if ( n_jet_store >= 6 ){ 
                  ++jetIdx;
                  jetLQMotherIdx = getJetLQMotherIdx(c_genJet_all, c_pfjet_final, jetRawIndicesMap[jetIdx], c_genQuark_hardProcess);
                  TLorentzVector t_ele1jet6 = t_ele1 + t_jet6;
                  FillUserTH1D("deltaREle1Jets", t_ele1.DeltaR ( t_jet6 ), genWeight);
                  if(ele1LQMotherIdx == jetLQMotherIdx)
                    FillUserTH1D("deltaR_Ele1JetSameLQ", t_ele1.DeltaR ( t_jet6 ), genWeight);
                }
              }
            }
          }
    ...
    if ( n_ele_store >= 2 ){
      ...
      if ( n_jet_store >= 1 ){ 
        ...
        if(ele2LQMotherIdx == jetLQMotherIdx) {
          FillUserTH1D("deltaR_Ele2JetSameLQ", t_ele2.DeltaR ( t_jet1 ), genWeight);
          FillUserTH2D("deltaREle2JetVsPt_sameLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet1 ), genWeight);
          if(ele2PassHeep)
            FillUserTH2D("deltaREle2JetVsPt_passHEEP_sameLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet1 ), genWeight);
          else
            FillUserTH2D("deltaREle2JetVsPt_failHEEP_sameLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet1 ), genWeight);
        }
        else if(jetLQMotherIdx > -1) {
          FillUserTH2D("deltaREle2JetVsPt_diffLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet1 ), genWeight);
          if(ele2PassHeep)
            FillUserTH2D("deltaREle2JetVsPt_passHEEP_diffLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet1 ), genWeight);
          else
            FillUserTH2D("deltaREle2JetVsPt_failHEEP_diffLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet1 ), genWeight);
        }
        else {
          FillUserTH2D("deltaREle2JetVsPt_noLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet1 ), genWeight);
          if(ele2PassHeep)
            FillUserTH2D("deltaREle2JetVsPt_passHEEP_noLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet1 ), genWeight);
          else
            FillUserTH2D("deltaREle2JetVsPt_failHEEP_noLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet1 ), genWeight);
        }
        if(closestJetIndex != jetRawIndicesMap[jetIdx]) {
          if(ele2LQMotherIdx == jetLQMotherIdx) {
            FillUserTH1D("deltaR_Ele2JetSameLQ_excludeClosestJet", t_ele2.DeltaR ( t_jet1 ), genWeight);
            FillUserTH2D("deltaREle2JetVsPt_sameLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet1 ), genWeight);
            if(ele2PassHeep)
              FillUserTH2D("deltaREle2JetVsPt_passHEEP_sameLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet1 ), genWeight);
            else
              FillUserTH2D("deltaREle2JetVsPt_failHEEP_sameLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet1 ), genWeight);
          }
          else if(jetLQMotherIdx > -1) {
            FillUserTH2D("deltaREle2JetVsPt_diffLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet1 ), genWeight);
            if(ele2PassHeep)
              FillUserTH2D("deltaREle2JetVsPt_passHEEP_diffLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet1 ), genWeight);
            else
              FillUserTH2D("deltaREle2JetVsPt_failHEEP_diffLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet1 ), genWeight);
          }
          else {
            FillUserTH2D("deltaREle2JetVsPt_noLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet1 ), genWeight);
            if(ele2PassHeep)
              FillUserTH2D("deltaREle2JetVsPt_passHEEP_noLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet1 ), genWeight);
            else
              FillUserTH2D("deltaREle2JetVsPt_failHEEP_noLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet1 ), genWeight);
          }
        }
        if(ele2PassEt) {
          FillUserTH1D("deltaREle2Jets_highET", t_ele2.DeltaR ( t_jet1 ), genWeight);
          if(closestJetIndex != jetRawIndicesMap[jetIdx]) {
            FillUserTH1D("deltaREle2Jets_highET_ExcludeClosestJet", t_ele2.DeltaR ( t_jet1 ), genWeight);
            if(ele2PassHeep)
              FillUserTH1D("deltaREle2PassHEEPJets_highET_ExcludeClosestJet", t_ele2.DeltaR ( t_jet1 ), genWeight);
            else if(!ele2PassHeepCaloIso) {
              FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_highETExcludeClosestJet", t_ele2.DeltaR ( t_jet1 ), genWeight);
              if(jetLQMotherIdx == ele2LQMotherIdx)
                FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_highETExcludeClosestJet_sameLQMother", t_ele2.DeltaR ( t_jet1 ), genWeight);
              else if(jetLQMotherIdx > -1)
                FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_highETExcludeClosestJet_diffLQMother", t_ele2.DeltaR ( t_jet1 ), genWeight);
              else
                FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_highETExcludeClosestJet_noLQMother", t_ele2.DeltaR ( t_jet1 ), genWeight);
            }
          }
          if(isElectronBarrel(ele2.SCEta()))
            FillUserTH1D("deltaREle2Jets_highET_barrel", t_ele2.DeltaR ( t_jet1 ), genWeight);
          else if(isElectronEndcap(ele2.SCEta()))
            FillUserTH1D("deltaREle2Jets_highET_endcap", t_ele2.DeltaR ( t_jet1 ), genWeight);
        }
        else {
          FillUserTH1D("deltaREle2Jets_lowET", t_ele2.DeltaR ( t_jet1 ), genWeight);
          if(closestJetIndex != jetRawIndicesMap[jetIdx]) {
            FillUserTH1D("deltaREle2Jets_lowET_ExcludeClosestJet", t_ele2.DeltaR ( t_jet1 ), genWeight);
            if(ele2PassHeep)
              FillUserTH1D("deltaREle2PassHEEPJets_lowET_ExcludeClosestJet", t_ele2.DeltaR ( t_jet1 ), genWeight);
            else if(!ele2PassHeepCaloIso) {
              FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_lowETExcludeClosestJet", t_ele2.DeltaR ( t_jet1 ), genWeight);
              if(jetLQMotherIdx == ele2LQMotherIdx)
                FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_lowETExcludeClosestJet_sameLQMother", t_ele2.DeltaR ( t_jet1 ), genWeight);
              else if(jetLQMotherIdx > -1)
                FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_lowETExcludeClosestJet_diffLQMother", t_ele2.DeltaR ( t_jet1 ), genWeight);
              else
                FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_lowETExcludeClosestJet_noLQMother", t_ele2.DeltaR ( t_jet1 ), genWeight);
            }
            if(!ele2PassHeep) {
          ...
          if(isElectronBarrel(ele2.SCEta()))
            FillUserTH1D("deltaREle2Jets_lowET_barrel", t_ele2.DeltaR ( t_jet1 ), genWeight);
          else if(isElectronEndcap(ele2.SCEta()))
            FillUserTH1D("deltaREle2Jets_lowET_endcap", t_ele2.DeltaR ( t_jet1 ), genWeight);
        }
        if(closestJetIndex != jetRawIndicesMap[jetIdx]) {
          FillUserTH1D("deltaREle2JetsExcludeClosestJet", t_ele2.DeltaR ( t_jet1 ), genWeight);
          if(ele2PassHeep)
            FillUserTH1D("deltaREle2PassHEEPJetsExcludeClosestJet", t_ele2.DeltaR ( t_jet1 ), genWeight);
          else if(!ele2PassHeepCaloIso)
            FillUserTH1D("deltaREle2FailHEEPCaloIsoJetsExcludeClosestJet", t_ele2.DeltaR ( t_jet1 ), genWeight);
        }
        FillUserTH1D("deltaREle2Jets", t_ele2.DeltaR ( t_jet1 ), genWeight);
        if(ele2PassHeep) {
          FillUserTH1D("deltaREleJetsPassHEEP", t_ele1.DeltaR ( t_jet1 ), genWeight);
          if(isElectronBarrel(ele2.SCEta()))
            FillUserTH1D("deltaREleJetsBarrelPassHEEP", t_ele1.DeltaR ( t_jet1 ), genWeight);
        ...
        if ( n_jet_store >= 2 ){ 
          ...
          if(ele2LQMotherIdx == jetLQMotherIdx) {
            FillUserTH1D("deltaR_Ele2JetSameLQ", t_ele2.DeltaR ( t_jet2 ), genWeight);
            FillUserTH2D("deltaREle2JetVsPt_sameLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet2 ), genWeight);
            if(ele2PassHeep)
              FillUserTH2D("deltaREle2JetVsPt_passHEEP_sameLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet2 ), genWeight);
            else
              FillUserTH2D("deltaREle2JetVsPt_failHEEP_sameLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet2 ), genWeight);
          }
          else if(jetLQMotherIdx > -1) {
            FillUserTH2D("deltaREle2JetVsPt_diffLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet2 ), genWeight);
            if(ele2PassHeep)
              FillUserTH2D("deltaREle2JetVsPt_passHEEP_diffLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet2 ), genWeight);
            else
              FillUserTH2D("deltaREle2JetVsPt_failHEEP_diffLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet2 ), genWeight);
          }
          else {
            FillUserTH2D("deltaREle2JetVsPt_noLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet2 ), genWeight);
            if(ele2PassHeep)
              FillUserTH2D("deltaREle2JetVsPt_passHEEP_noLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet2 ), genWeight);
            else
              FillUserTH2D("deltaREle2JetVsPt_failHEEP_noLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet2 ), genWeight);
          }
          if(closestJetIndex != jetRawIndicesMap[jetIdx]) {
            if(ele2LQMotherIdx == jetLQMotherIdx) {
              FillUserTH1D("deltaR_Ele2JetSameLQ_excludeClosestJet", t_ele2.DeltaR ( t_jet2 ), genWeight);
              FillUserTH2D("deltaREle2JetVsPt_sameLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet2 ), genWeight);
              if(ele2PassHeep)
                FillUserTH2D("deltaREle2JetVsPt_passHEEP_sameLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet2 ), genWeight);
              else
                FillUserTH2D("deltaREle2JetVsPt_failHEEP_sameLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet2 ), genWeight);
            }
            else if(jetLQMotherIdx > -1) {
              FillUserTH2D("deltaREle2JetVsPt_diffLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet2 ), genWeight);
              if(ele2PassHeep)
                FillUserTH2D("deltaREle2JetVsPt_passHEEP_diffLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet2 ), genWeight);
              else
                FillUserTH2D("deltaREle2JetVsPt_failHEEP_diffLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet2 ), genWeight);
            }
            else {
              FillUserTH2D("deltaREle2JetVsPt_noLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet2 ), genWeight);
              if(ele2PassHeep)
                FillUserTH2D("deltaREle2JetVsPt_passHEEP_noLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet2 ), genWeight);
              else
                FillUserTH2D("deltaREle2JetVsPt_failHEEP_noLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet2 ), genWeight);
            }
          }
          if(ele2PassEt) {
            FillUserTH1D("deltaREle2Jets_highET", t_ele2.DeltaR ( t_jet2 ), genWeight);
            if(closestJetIndex != jetRawIndicesMap[1]) {
              FillUserTH1D("deltaREle2Jets_highET_ExcludeClosestJet", t_ele2.DeltaR ( t_jet2 ), genWeight);
              if(ele2PassHeep)
                FillUserTH1D("deltaREle2PassHEEPJets_highET_ExcludeClosestJet", t_ele2.DeltaR ( t_jet2 ), genWeight);
              else if(!ele2PassHeepCaloIso) {
                FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_highETExcludeClosestJet", t_ele2.DeltaR ( t_jet2 ), genWeight);
                if(jetLQMotherIdx == ele2LQMotherIdx)
                  FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_highETExcludeClosestJet_sameLQMother", t_ele2.DeltaR ( t_jet2 ), genWeight);
                else if(jetLQMotherIdx > -1)
                  FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_highETExcludeClosestJet_diffLQMother", t_ele2.DeltaR ( t_jet2 ), genWeight);
                else
                  FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_highETExcludeClosestJet_noLQMother", t_ele2.DeltaR ( t_jet2 ), genWeight);
              }
            }
            if(isElectronBarrel(ele2.SCEta()))
              FillUserTH1D("deltaREle2Jets_highET_barrel", t_ele2.DeltaR ( t_jet2 ), genWeight);
            else if(isElectronEndcap(ele2.SCEta()))
              FillUserTH1D("deltaREle2Jets_highET_endcap", t_ele2.DeltaR ( t_jet2 ), genWeight);
          }
          else {
            FillUserTH1D("deltaREle2Jets_lowET", t_ele2.DeltaR ( t_jet2 ), genWeight);
            if(closestJetIndex != jetRawIndicesMap[1]) {
              FillUserTH1D("deltaREle2Jets_lowET_ExcludeClosestJet", t_ele2.DeltaR ( t_jet2 ), genWeight);
              if(ele2PassHeep)
                FillUserTH1D("deltaREle2PassHEEPJets_lowET_ExcludeClosestJet", t_ele2.DeltaR ( t_jet2 ), genWeight);
              else if(!ele2PassHeepCaloIso) {
                FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_lowETExcludeClosestJet", t_ele2.DeltaR ( t_jet2 ), genWeight);
                if(jetLQMotherIdx == ele2LQMotherIdx)
                  FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_lowETExcludeClosestJet_sameLQMother", t_ele2.DeltaR ( t_jet2 ), genWeight);
                else if(jetLQMotherIdx > -1)
                  FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_lowETExcludeClosestJet_diffLQMother", t_ele2.DeltaR ( t_jet2 ), genWeight);
                else
                  FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_lowETExcludeClosestJet_noLQMother", t_ele2.DeltaR ( t_jet2 ), genWeight);
              }
              if(!ele2PassHeep) {
            ...
            if(isElectronBarrel(ele2.SCEta()))
              FillUserTH1D("deltaREle2Jets_lowET_barrel", t_ele2.DeltaR ( t_jet2 ), genWeight);
            else if(isElectronEndcap(ele2.SCEta()))
              FillUserTH1D("deltaREle2Jets_lowET_endcap", t_ele2.DeltaR ( t_jet2 ), genWeight);
          }
          fillVariableWithValue("DR_Ele2Jet2", t_ele2.DeltaR ( t_jet2 ), genWeight);
          fillVariableWithValue("M_e2j2"     , t_ele2jet2.M());
          fillVariableWithValue("sT_eejj"    , t_ele1.Pt() + t_ele2.Pt() + t_jet1.Pt() + t_jet2.Pt());
          if(closestJetIndex != jetRawIndicesMap[1]) {
            FillUserTH1D("deltaREle2JetsExcludeClosestJet", t_ele2.DeltaR ( t_jet2 ), genWeight);
            if(ele2PassHeep)
              FillUserTH1D("deltaREle2PassHEEPJetsExcludeClosestJet", t_ele2.DeltaR ( t_jet2 ), genWeight);
            else if(!ele2PassHeepCaloIso)
              FillUserTH1D("deltaREle2FailHEEPCaloIsoJetsExcludeClosestJet", t_ele2.DeltaR ( t_jet2 ), genWeight);
          }
          FillUserTH1D("deltaREle2Jets", t_ele2.DeltaR ( t_jet2 ), genWeight);
          if(ele2PassHeep) {
            FillUserTH1D("deltaREleJetsPassHEEP", t_ele1.DeltaR ( t_jet2 ), genWeight);
            if(isElectronBarrel(ele2.SCEta()))
              FillUserTH1D("deltaREleJetsBarrelPassHEEP", t_ele1.DeltaR ( t_jet2 ), genWeight);
          ...
          if ( n_jet_store >= 3 ){ 
            ...
            if(ele2LQMotherIdx == jetLQMotherIdx) {
              FillUserTH1D("deltaR_Ele2JetSameLQ", t_ele2.DeltaR ( t_jet3 ), genWeight);
              FillUserTH2D("deltaREle2JetVsPt_sameLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet3 ), genWeight);
              if(ele2PassHeep)
                FillUserTH2D("deltaREle2JetVsPt_passHEEP_sameLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet3 ), genWeight);
              else
                FillUserTH2D("deltaREle2JetVsPt_failHEEP_sameLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet3 ), genWeight);
            }
            else if(jetLQMotherIdx > -1) {
              FillUserTH2D("deltaREle2JetVsPt_diffLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet3 ), genWeight);
              if(ele2PassHeep)
                FillUserTH2D("deltaREle2JetVsPt_passHEEP_diffLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet3 ), genWeight);
              else
                FillUserTH2D("deltaREle2JetVsPt_failHEEP_diffLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet3 ), genWeight);
            }
            else {
              FillUserTH2D("deltaREle2JetVsPt_noLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet3 ), genWeight);
              if(ele2PassHeep)
                FillUserTH2D("deltaREle2JetVsPt_passHEEP_noLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet3 ), genWeight);
              else
                FillUserTH2D("deltaREle2JetVsPt_failHEEP_noLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet3 ), genWeight);
            }
            if(closestJetIndex != jetRawIndicesMap[jetIdx]) {
              if(ele2LQMotherIdx == jetLQMotherIdx) {
                FillUserTH1D("deltaR_Ele2JetSameLQ_excludeClosestJet", t_ele2.DeltaR ( t_jet3 ), genWeight);
                FillUserTH2D("deltaREle2JetVsPt_sameLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet3 ), genWeight);
                if(ele2PassHeep)
                  FillUserTH2D("deltaREle2JetVsPt_passHEEP_sameLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet3 ), genWeight);
                else
                  FillUserTH2D("deltaREle2JetVsPt_failHEEP_sameLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet3 ), genWeight);
              }
              else if(jetLQMotherIdx > -1) {
                FillUserTH2D("deltaREle2JetVsPt_diffLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet3 ), genWeight);
                if(ele2PassHeep)
                  FillUserTH2D("deltaREle2JetVsPt_passHEEP_diffLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet3 ), genWeight);
                else
                  FillUserTH2D("deltaREle2JetVsPt_failHEEP_diffLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet3 ), genWeight);
              }
              else {
                FillUserTH2D("deltaREle2JetVsPt_noLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet3 ), genWeight);
                if(ele2PassHeep)
                  FillUserTH2D("deltaREle2JetVsPt_passHEEP_noLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet3 ), genWeight);
                else
                  FillUserTH2D("deltaREle2JetVsPt_failHEEP_noLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet3 ), genWeight);
              }
            }
            if(ele2PassEt) {
              FillUserTH1D("deltaREle2Jets_highET", t_ele2.DeltaR ( t_jet3 ), genWeight);
              if(closestJetIndex != jetRawIndicesMap[2]) {
                FillUserTH1D("deltaREle2Jets_highET_ExcludeClosestJet", t_ele2.DeltaR ( t_jet3 ), genWeight);
                if(ele2PassHeep)
                  FillUserTH1D("deltaREle2PassHEEPJets_highET_ExcludeClosestJet", t_ele2.DeltaR ( t_jet3 ), genWeight);
                else if(!ele2PassHeepCaloIso) {
                  FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_highETExcludeClosestJet", t_ele2.DeltaR ( t_jet3 ), genWeight);
                  if(jetLQMotherIdx == ele2LQMotherIdx)
                    FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_highETExcludeClosestJet_sameLQMother", t_ele2.DeltaR ( t_jet3 ), genWeight);
                  else if(jetLQMotherIdx > -1)
                    FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_highETExcludeClosestJet_diffLQMother", t_ele2.DeltaR ( t_jet3 ), genWeight);
                  else
                    FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_highETExcludeClosestJet_noLQMother", t_ele2.DeltaR ( t_jet3 ), genWeight);
                }
              }
              if(isElectronBarrel(ele2.SCEta()))
                FillUserTH1D("deltaREle2Jets_highET_barrel", t_ele2.DeltaR ( t_jet3 ), genWeight);
              else if(isElectronEndcap(ele2.SCEta()))
                FillUserTH1D("deltaREle2Jets_highET_endcap", t_ele2.DeltaR ( t_jet3 ), genWeight);
            }
            else {
              FillUserTH1D("deltaREle2Jets_lowET", t_ele2.DeltaR ( t_jet3 ), genWeight);
              if(closestJetIndex != jetRawIndicesMap[2]) {
                FillUserTH1D("deltaREle2Jets_lowET_ExcludeClosestJet", t_ele2.DeltaR ( t_jet3 ), genWeight);
                if(ele2PassHeep)
                  FillUserTH1D("deltaREle2PassHEEPJets_lowET_ExcludeClosestJet", t_ele2.DeltaR ( t_jet3 ), genWeight);
                else if(!ele2PassHeepCaloIso) {
                  FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_lowETExcludeClosestJet", t_ele2.DeltaR ( t_jet3 ), genWeight);
                  if(jetLQMotherIdx == ele2LQMotherIdx)
                    FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_lowETExcludeClosestJet_sameLQMother", t_ele2.DeltaR ( t_jet3 ), genWeight);
                  else if(jetLQMotherIdx > -1)
                    FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_lowETExcludeClosestJet_diffLQMother", t_ele2.DeltaR ( t_jet3 ), genWeight);
                  else
                    FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_lowETExcludeClosestJet_noLQMother", t_ele2.DeltaR ( t_jet3 ), genWeight);
                }
                if(!ele2PassHeep) {
              ...
              if(isElectronBarrel(ele2.SCEta()))
                FillUserTH1D("deltaREle2Jets_lowET_barrel", t_ele2.DeltaR ( t_jet3 ), genWeight);
              else if(isElectronEndcap(ele2.SCEta()))
                FillUserTH1D("deltaREle2Jets_lowET_endcap", t_ele2.DeltaR ( t_jet3 ), genWeight);
            }
            if(closestJetIndex != jetRawIndicesMap[2]) {
              FillUserTH1D("deltaREle2JetsExcludeClosestJet", t_ele2.DeltaR ( t_jet3 ), genWeight);
              if(ele2PassHeep)
                FillUserTH1D("deltaREle2PassHEEPJetsExcludeClosestJet", t_ele2.DeltaR ( t_jet3 ), genWeight);
              else if(!ele2PassHeepCaloIso)
                FillUserTH1D("deltaREle2FailHEEPCaloIsoJetsExcludeClosestJet", t_ele2.DeltaR ( t_jet3 ), genWeight);
            }
            FillUserTH1D("deltaREle2Jets", t_ele2.DeltaR ( t_jet3 ), genWeight);
            if ( n_jet_store >= 4 ){ 
              ++jetIdx;
              jetLQMotherIdx = getJetLQMotherIdx(c_genJet_all, c_pfjet_final, jetRawIndicesMap[jetIdx], c_genQuark_hardProcess);
              if(ele2LQMotherIdx == jetLQMotherIdx) {
                FillUserTH1D("deltaR_Ele2JetSameLQ", t_ele2.DeltaR ( t_jet4 ), genWeight);
                FillUserTH2D("deltaREle2JetVsPt_sameLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet4 ), genWeight);
                if(ele2PassHeep)
                  FillUserTH2D("deltaREle2JetVsPt_passHEEP_sameLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet4 ), genWeight);
                else
                  FillUserTH2D("deltaREle2JetVsPt_failHEEP_sameLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet4 ), genWeight);
              }
              else if(jetLQMotherIdx > -1) {
                FillUserTH2D("deltaREle2JetVsPt_diffLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet4 ), genWeight);
                if(ele2PassHeep)
                  FillUserTH2D("deltaREle2JetVsPt_passHEEP_diffLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet4 ), genWeight);
                else
                  FillUserTH2D("deltaREle2JetVsPt_failHEEP_diffLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet4 ), genWeight);
              }
              else {
                FillUserTH2D("deltaREle2JetVsPt_noLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet4 ), genWeight);
                if(ele2PassHeep)
                  FillUserTH2D("deltaREle2JetVsPt_passHEEP_noLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet4 ), genWeight);
                else
                  FillUserTH2D("deltaREle2JetVsPt_failHEEP_noLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet4 ), genWeight);
              }
              if(closestJetIndex != jetRawIndicesMap[jetIdx]) {
                if(ele2LQMotherIdx == jetLQMotherIdx) {
                  FillUserTH1D("deltaR_Ele2JetSameLQ_excludeClosestJet", t_ele2.DeltaR ( t_jet4 ), genWeight);
                  FillUserTH2D("deltaREle2JetVsPt_sameLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet4 ), genWeight);
                  if(ele2PassHeep)
                    FillUserTH2D("deltaREle2JetVsPt_passHEEP_sameLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet4 ), genWeight);
                  else
                    FillUserTH2D("deltaREle2JetVsPt_failHEEP_sameLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet4 ), genWeight);
                }
                else if(jetLQMotherIdx > -1) {
                  FillUserTH2D("deltaREle2JetVsPt_diffLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet4 ), genWeight);
                  if(ele2PassHeep)
                    FillUserTH2D("deltaREle2JetVsPt_passHEEP_diffLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet4 ), genWeight);
                  else
                    FillUserTH2D("deltaREle2JetVsPt_failHEEP_diffLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet4 ), genWeight);
                }
                else {
                  FillUserTH2D("deltaREle2JetVsPt_noLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet4 ), genWeight);
                  if(ele2PassHeep)
                    FillUserTH2D("deltaREle2JetVsPt_passHEEP_noLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet4 ), genWeight);
                  else
                    FillUserTH2D("deltaREle2JetVsPt_failHEEP_noLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet4 ), genWeight);
                }
              }
              if(ele2PassEt) {
                FillUserTH1D("deltaREle2Jets_highET", t_ele2.DeltaR ( t_jet4 ), genWeight);
                if(closestJetIndex != jetRawIndicesMap[3]) {
                  FillUserTH1D("deltaREle2Jets_highET_ExcludeClosestJet", t_ele2.DeltaR ( t_jet4 ), genWeight);
                  if(ele2PassHeep)
                    FillUserTH1D("deltaREle2PassHEEPJets_highET_ExcludeClosestJet", t_ele2.DeltaR ( t_jet4 ), genWeight);
                  else if(!ele2PassHeepCaloIso) {
                    FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_highETExcludeClosestJet", t_ele2.DeltaR ( t_jet4 ), genWeight);
                    if(jetLQMotherIdx == ele2LQMotherIdx)
                      FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_highETExcludeClosestJet_sameLQMother", t_ele2.DeltaR ( t_jet4 ), genWeight);
                    else if(jetLQMotherIdx > -1)
                      FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_highETExcludeClosestJet_diffLQMother", t_ele2.DeltaR ( t_jet4 ), genWeight);
                    else
                      FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_highETExcludeClosestJet_noLQMother", t_ele2.DeltaR ( t_jet4 ), genWeight);
                  }
                }
                if(isElectronBarrel(ele2.SCEta()))
                  FillUserTH1D("deltaREle2Jets_highET_barrel", t_ele2.DeltaR ( t_jet4 ), genWeight);
                else if(isElectronEndcap(ele2.SCEta()))
                  FillUserTH1D("deltaREle2Jets_highET_endcap", t_ele2.DeltaR ( t_jet4 ), genWeight);
              }
              else {
                FillUserTH1D("deltaREle2Jets_lowET", t_ele2.DeltaR ( t_jet4 ), genWeight);
                if(closestJetIndex != jetRawIndicesMap[3]) {
                  FillUserTH1D("deltaREle2Jets_lowET_ExcludeClosestJet", t_ele2.DeltaR ( t_jet4 ), genWeight);
                  if(ele2PassHeep)
                    FillUserTH1D("deltaREle2PassHEEPJets_lowET_ExcludeClosestJet", t_ele2.DeltaR ( t_jet4 ), genWeight);
                  else if(!ele2PassHeepCaloIso) {
                    FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_lowETExcludeClosestJet", t_ele2.DeltaR ( t_jet4 ), genWeight);
                    if(jetLQMotherIdx == ele2LQMotherIdx)
                      FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_lowETExcludeClosestJet_sameLQMother", t_ele2.DeltaR ( t_jet4 ), genWeight);
                    else if(jetLQMotherIdx > -1)
                      FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_lowETExcludeClosestJet_diffLQMother", t_ele2.DeltaR ( t_jet4 ), genWeight);
                    else
                      FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_lowETExcludeClosestJet_noLQMother", t_ele2.DeltaR ( t_jet4 ), genWeight);
                  }
                  if(!ele2PassHeep) {
                ...
                if(isElectronBarrel(ele2.SCEta()))
                  FillUserTH1D("deltaREle2Jets_lowET_barrel", t_ele2.DeltaR ( t_jet4 ), genWeight);
                else if(isElectronEndcap(ele2.SCEta()))
                  FillUserTH1D("deltaREle2Jets_lowET_endcap", t_ele2.DeltaR ( t_jet4 ), genWeight);
              }
              TLorentzVector t_ele2jet4 = t_ele2 + t_jet4;
              if(closestJetIndex != jetRawIndicesMap[3]) {
                FillUserTH1D("deltaREle2JetsExcludeClosestJet", t_ele2.DeltaR ( t_jet4 ), genWeight);
                if(ele2PassHeep)
                  FillUserTH1D("deltaREle2PassHEEPJetsExcludeClosestJet", t_ele2.DeltaR ( t_jet4 ), genWeight);
                else if(!ele2PassHeepCaloIso)
                  FillUserTH1D("deltaREle2FailHEEPCaloIsoJetsExcludeClosestJet", t_ele2.DeltaR ( t_jet4 ), genWeight);
              }
              FillUserTH1D("deltaREle2Jets", t_ele2.DeltaR ( t_jet4 ), genWeight);
              if ( n_jet_store >= 5 ){ 
                ++jetIdx;
                jetLQMotherIdx = getJetLQMotherIdx(c_genJet_all, c_pfjet_final, jetRawIndicesMap[jetIdx], c_genQuark_hardProcess);
                TLorentzVector t_ele2jet5 = t_ele2 + t_jet5;
                if(ele2LQMotherIdx == jetLQMotherIdx) {
                  FillUserTH1D("deltaR_Ele2JetSameLQ", t_ele2.DeltaR ( t_jet5 ), genWeight);
                  FillUserTH2D("deltaREle2JetVsPt_sameLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet5 ), genWeight);
                  if(ele2PassHeep)
                    FillUserTH2D("deltaREle2JetVsPt_passHEEP_sameLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet5 ), genWeight);
                  else
                    FillUserTH2D("deltaREle2JetVsPt_failHEEP_sameLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet5 ), genWeight);
                }
                else if(jetLQMotherIdx > -1) {
                  FillUserTH2D("deltaREle2JetVsPt_diffLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet5 ), genWeight);
                  if(ele2PassHeep)
                    FillUserTH2D("deltaREle2JetVsPt_passHEEP_diffLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet5 ), genWeight);
                  else
                    FillUserTH2D("deltaREle2JetVsPt_failHEEP_diffLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet5 ), genWeight);
                }
                else {
                  FillUserTH2D("deltaREle2JetVsPt_noLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet5 ), genWeight);
                  if(ele2PassHeep)
                    FillUserTH2D("deltaREle2JetVsPt_passHEEP_noLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet5 ), genWeight);
                  else
                    FillUserTH2D("deltaREle2JetVsPt_failHEEP_noLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet5 ), genWeight);
                }
                if(closestJetIndex != jetRawIndicesMap[jetIdx]) {
                  if(ele2LQMotherIdx == jetLQMotherIdx) {
                    FillUserTH1D("deltaR_Ele2JetSameLQ_excludeClosestJet", t_ele2.DeltaR ( t_jet5 ), genWeight);
                    FillUserTH2D("deltaREle2JetVsPt_sameLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet5 ), genWeight);
                    if(ele2PassHeep)
                      FillUserTH2D("deltaREle2JetVsPt_passHEEP_sameLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet5 ), genWeight);
                    else
                      FillUserTH2D("deltaREle2JetVsPt_failHEEP_sameLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet5 ), genWeight);
                  }
                  else if(jetLQMotherIdx > -1) {
                    FillUserTH2D("deltaREle2JetVsPt_diffLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet5 ), genWeight);
                    if(ele2PassHeep)
                      FillUserTH2D("deltaREle2JetVsPt_passHEEP_diffLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet5 ), genWeight);
                    else
                      FillUserTH2D("deltaREle2JetVsPt_failHEEP_diffLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet5 ), genWeight);
                  }
                  else {
                    FillUserTH2D("deltaREle2JetVsPt_noLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet5 ), genWeight);
                    if(ele2PassHeep)
                      FillUserTH2D("deltaREle2JetVsPt_passHEEP_noLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet5 ), genWeight);
                    else
                      FillUserTH2D("deltaREle2JetVsPt_failHEEP_noLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet5 ), genWeight);
                  }
                }
                if(ele2PassEt) {
                  FillUserTH1D("deltaREle2Jets_highET", t_ele2.DeltaR ( t_jet5 ), genWeight);
                  if(closestJetIndex != jetRawIndicesMap[4]) {
                    FillUserTH1D("deltaREle2Jets_highET_ExcludeClosestJet", t_ele2.DeltaR ( t_jet5 ), genWeight);
                    if(ele2PassHeep)
                      FillUserTH1D("deltaREle2PassHEEPJets_highET_ExcludeClosestJet", t_ele2.DeltaR ( t_jet5 ), genWeight);
                    else if(!ele2PassHeepCaloIso) {
                      FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_highETExcludeClosestJet", t_ele2.DeltaR ( t_jet5 ), genWeight);
                      if(jetLQMotherIdx == ele2LQMotherIdx)
                        FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_highETExcludeClosestJet_sameLQMother", t_ele2.DeltaR ( t_jet5 ), genWeight);
                      else if(jetLQMotherIdx > -1)
                        FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_highETExcludeClosestJet_diffLQMother", t_ele2.DeltaR ( t_jet5 ), genWeight);
                      else
                        FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_highETExcludeClosestJet_noLQMother", t_ele2.DeltaR ( t_jet5 ), genWeight);
                    }
                  }
                  if(isElectronBarrel(ele2.SCEta()))
                    FillUserTH1D("deltaREle2Jets_highET_barrel", t_ele2.DeltaR ( t_jet5 ), genWeight);
                  else if(isElectronEndcap(ele2.SCEta()))
                    FillUserTH1D("deltaREle2Jets_highET_endcap", t_ele2.DeltaR ( t_jet5 ), genWeight);
                }
                else {
                  FillUserTH1D("deltaREle2Jets_lowET", t_ele2.DeltaR ( t_jet5 ), genWeight);
                  if(closestJetIndex != jetRawIndicesMap[4]) {
                    FillUserTH1D("deltaREle2Jets_lowET_ExcludeClosestJet", t_ele2.DeltaR ( t_jet5 ), genWeight);
                    if(ele2PassHeep)
                      FillUserTH1D("deltaREle2PassHEEPJets_lowET_ExcludeClosestJet", t_ele2.DeltaR ( t_jet5 ), genWeight);
                    else if(!ele2PassHeepCaloIso) {
                      FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_lowETExcludeClosestJet", t_ele2.DeltaR ( t_jet5 ), genWeight);
                      if(jetLQMotherIdx == ele2LQMotherIdx)
                        FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_lowETExcludeClosestJet_sameLQMother", t_ele2.DeltaR ( t_jet5 ), genWeight);
                      else if(jetLQMotherIdx > -1)
                        FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_lowETExcludeClosestJet_diffLQMother", t_ele2.DeltaR ( t_jet5 ), genWeight);
                      else
                        FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_lowETExcludeClosestJet_noLQMother", t_ele2.DeltaR ( t_jet5 ), genWeight);
                    }
                    if(!ele2PassHeep) {
                  ...
                  if(isElectronBarrel(ele2.SCEta()))
                    FillUserTH1D("deltaREle2Jets_lowET_barrel", t_ele2.DeltaR ( t_jet5 ), genWeight);
                  else if(isElectronEndcap(ele2.SCEta()))
                    FillUserTH1D("deltaREle2Jets_lowET_endcap", t_ele2.DeltaR ( t_jet5 ), genWeight);
                }
                if(closestJetIndex != jetRawIndicesMap[4]) {
                  FillUserTH1D("deltaREle2JetsExcludeClosestJet", t_ele2.DeltaR ( t_jet5 ), genWeight);
                  if(ele2PassHeep)
                    FillUserTH1D("deltaREle2PassHEEPJetsExcludeClosestJet", t_ele2.DeltaR ( t_jet5 ), genWeight);
                  else if(!ele2PassHeepCaloIso)
                    FillUserTH1D("deltaREle2FailHEEPCaloIsoJetsExcludeClosestJet", t_ele2.DeltaR ( t_jet5 ), genWeight);
                }
                FillUserTH1D("deltaREle2Jets", t_ele2.DeltaR ( t_jet5 ), genWeight);
                if ( n_jet_store >= 6 ){ 
                  ++jetIdx;
                  jetLQMotherIdx = getJetLQMotherIdx(c_genJet_all, c_pfjet_final, jetRawIndicesMap[jetIdx], c_genQuark_hardProcess);
                  TLorentzVector t_ele2jet6 = t_ele2 + t_jet6;
                  if(ele2LQMotherIdx == jetLQMotherIdx) {
                    FillUserTH1D("deltaR_Ele2JetSameLQ", t_ele2.DeltaR ( t_jet6 ), genWeight);
                    FillUserTH2D("deltaREle2JetVsPt_sameLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet6 ), genWeight);
                    if(ele2PassHeep)
                      FillUserTH2D("deltaREle2JetVsPt_passHEEP_sameLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet6 ), genWeight);
                    else
                      FillUserTH2D("deltaREle2JetVsPt_failHEEP_sameLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet6 ), genWeight);
                  }
                  else if(jetLQMotherIdx > -1) {
                    FillUserTH2D("deltaREle2JetVsPt_diffLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet6 ), genWeight);
                    if(ele2PassHeep)
                      FillUserTH2D("deltaREle2JetVsPt_passHEEP_diffLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet6 ), genWeight);
                    else
                      FillUserTH2D("deltaREle2JetVsPt_failHEEP_diffLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet6 ), genWeight);
                  }
                  else {
                    FillUserTH2D("deltaREle2JetVsPt_noLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet6 ), genWeight);
                    if(ele2PassHeep)
                      FillUserTH2D("deltaREle2JetVsPt_passHEEP_noLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet6 ), genWeight);
                    else
                      FillUserTH2D("deltaREle2JetVsPt_failHEEP_noLQmother", ele2.SCEt(), t_ele2.DeltaR ( t_jet6 ), genWeight);
                  }
                  if(closestJetIndex != jetRawIndicesMap[jetIdx]) {
                    if(ele2LQMotherIdx == jetLQMotherIdx) {
                      FillUserTH1D("deltaR_Ele2JetSameLQ_excludeClosestJet", t_ele2.DeltaR ( t_jet6 ), genWeight);
                      FillUserTH2D("deltaREle2JetVsPt_sameLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet6 ), genWeight);
                      if(ele2PassHeep)
                        FillUserTH2D("deltaREle2JetVsPt_passHEEP_sameLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet6 ), genWeight);
                      else
                        FillUserTH2D("deltaREle2JetVsPt_failHEEP_sameLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet6 ), genWeight);
                    }
                    else if(jetLQMotherIdx > -1) {
                      FillUserTH2D("deltaREle2JetVsPt_diffLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet6 ), genWeight);
                      if(ele2PassHeep)
                        FillUserTH2D("deltaREle2JetVsPt_passHEEP_diffLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet6 ), genWeight);
                      else
                        FillUserTH2D("deltaREle2JetVsPt_failHEEP_diffLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet6 ), genWeight);
                    }
                    else {
                      FillUserTH2D("deltaREle2JetVsPt_noLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet6 ), genWeight);
                      if(ele2PassHeep)
                        FillUserTH2D("deltaREle2JetVsPt_passHEEP_noLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet6 ), genWeight);
                      else
                        FillUserTH2D("deltaREle2JetVsPt_failHEEP_noLQmother_excludeClosestJet", ele2.SCEt(), t_ele2.DeltaR ( t_jet6 ), genWeight);
                    }
                  }
                  if(ele2PassEt) {
                    FillUserTH1D("deltaREle2Jets_highET", t_ele2.DeltaR ( t_jet6 ), genWeight);
                    if(closestJetIndex != jetRawIndicesMap[5]) {
                      FillUserTH1D("deltaREle2Jets_highET_ExcludeClosestJet", t_ele2.DeltaR ( t_jet6 ), genWeight);
                      if(ele2PassHeep)
                        FillUserTH1D("deltaREle2PassHEEPJets_highET_ExcludeClosestJet", t_ele2.DeltaR ( t_jet6 ), genWeight);
                      else if(!ele2PassHeepCaloIso) {
                        FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_highETExcludeClosestJet", t_ele2.DeltaR ( t_jet6 ), genWeight);
                        if(jetLQMotherIdx == ele2LQMotherIdx)
                          FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_highETExcludeClosestJet_sameLQMother", t_ele2.DeltaR ( t_jet6 ), genWeight);
                        else if(jetLQMotherIdx > -1)
                          FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_highETExcludeClosestJet_diffLQMother", t_ele2.DeltaR ( t_jet6 ), genWeight);
                        else
                          FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_highETExcludeClosestJet_noLQMother", t_ele2.DeltaR ( t_jet6 ), genWeight);
                      }
                    }
                    if(isElectronBarrel(ele2.SCEta()))
                      FillUserTH1D("deltaREle2Jets_highET_barrel", t_ele2.DeltaR ( t_jet6 ), genWeight);
                    else if(isElectronEndcap(ele2.SCEta()))
                      FillUserTH1D("deltaREle2Jets_highET_endcap", t_ele2.DeltaR ( t_jet6 ), genWeight);
                  }
                  else {
                    FillUserTH1D("deltaREle2Jets_lowET", t_ele2.DeltaR ( t_jet6 ), genWeight);
                    if(closestJetIndex != jetRawIndicesMap[5]) {
                      FillUserTH1D("deltaREle2Jets_lowET_ExcludeClosestJet", t_ele2.DeltaR ( t_jet6 ), genWeight);
                      if(ele2PassHeep)
                        FillUserTH1D("deltaREle2PassHEEPJets_lowET_ExcludeClosestJet", t_ele2.DeltaR ( t_jet6 ), genWeight);
                      else if(!ele2PassHeepCaloIso) {
                        FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_lowETExcludeClosestJet", t_ele2.DeltaR ( t_jet6 ), genWeight);
                        if(jetLQMotherIdx == ele2LQMotherIdx)
                          FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_lowETExcludeClosestJet_sameLQMother", t_ele2.DeltaR ( t_jet6 ), genWeight);
                        else if(jetLQMotherIdx > -1)
                          FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_lowETExcludeClosestJet_diffLQMother", t_ele2.DeltaR ( t_jet6 ), genWeight);
                        else
                          FillUserTH1D("deltaREle2FailHEEPCaloIsoJets_lowETExcludeClosestJet_noLQMother", t_ele2.DeltaR ( t_jet6 ), genWeight);
                      }
                      if(!ele2PassHeep) {
                    ...
                    if(isElectronBarrel(ele2.SCEta()))
                      FillUserTH1D("deltaREle2Jets_lowET_barrel", t_ele2.DeltaR ( t_jet6 ), genWeight);
                    else if(isElectronEndcap(ele2.SCEta()))
                      FillUserTH1D("deltaREle2Jets_lowET_endcap", t_ele2.DeltaR ( t_jet6 ), genWeight);
                  }
                  if(closestJetIndex != jetRawIndicesMap[5]) {
                    FillUserTH1D("deltaREle2JetsExcludeClosestJet", t_ele2.DeltaR ( t_jet6 ), genWeight);
                    if(ele2PassHeep)
                      FillUserTH1D("deltaREle2PassHEEPJetsExcludeClosestJet", t_ele2.DeltaR ( t_jet6 ), genWeight);
                    else if(!ele2PassHeepCaloIso)
                      FillUserTH1D("deltaREle2FailHEEPCaloIsoJetsExcludeClosestJet", t_ele2.DeltaR ( t_jet6 ), genWeight);
                  }
                  FillUserTH1D("deltaREle2Jets", t_ele2.DeltaR ( t_jet6 ), genWeight);
                }
              }
            }
          }
