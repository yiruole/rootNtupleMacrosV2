...
void analysisClass::Loop()
{
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) { // Begin of loop over events
	...
	if ( JES*CaloJetPt->at(ijet) < getPreCutValue1("jet_PtCut") ) continue;
    ...
    if( v_idx_jet_PtCut_noOverlap_ID.size() >= 1 ) 
      {
	fillVariableWithValue( "Pt1stJet_noOvrlp_ID", JES*CaloJetPt->at(v_idx_jet_PtCut_noOverlap_ID[0]) );
	...
	//PAS June 2010
	fillVariableWithValue( "Pt1stJet_PAS", JES*CaloJetPt->at(v_idx_jet_PtCut_noOverlap_ID[0]) );
    ...
    if( v_idx_jet_PtCut_noOverlap_ID.size() >= 2 ) 
      {
	fillVariableWithValue( "Pt2ndJet_noOvrlp_ID", JES*CaloJetPt->at(v_idx_jet_PtCut_noOverlap_ID[1]) );
	...
	//PAS June 2010
	fillVariableWithValue( "Pt2ndJet_PAS", JES*CaloJetPt->at(v_idx_jet_PtCut_noOverlap_ID[1]) );
	...
	jet2.SetPtEtaPhiM(JES*CaloJetPt->at(v_idx_jet_PtCut_noOverlap_ID[1]),
