...
void analysisClass::Loop()
{
  ...
  double jet_PtCut_2 =    getPreCutValue2("jet_PtCut");
  ...
  if(doGenLevelStudies)
    {
      ...
      CreateUserTH1D("h_DeltaR_recoJet_genJetMatched", 200,0,10);   
      CreateUserTH1D("h_PTrecoJet_Over_PTgenJetMatched", 100,0,5);   
      CreateUserTH2D("h2_EtaPhiMap_JetResponseLess0.6", 25, -5, 5, 18, -3.1416,+3.1416 );
      CreateUserTH1D("h_DeltaMET", 100, -250, 250 );
      CreateUserTH1D("h_MaxDeltaMET", 100, -250, 250 );
      CreateUserTH2D("h2_EtaPhiMap_JetWithLargestDeltaMET", 25, -5, 5, 18, -3.1416,+3.1416 );
      CreateUserTH2D("h2_indexJet_vs_MaxDeltaMET", 50, -250, 250, 15, 0, 15 );
      CreateUserTH1D("h_PTrecoJet_Over_PTgenJetMatched_PosMaxDeltaMET", 100,0,5);   
      CreateUserTH1D("h_PTrecoJet_Over_PTgenJetMatched_NegMaxDeltaMET", 100,0,5); 
      CreateUserTH1D("h_MaxDeltaMET__sT", 100, -250, 250 );
      CreateUserTH2D("h2_EtaPhiMap_JetWithLargestDeltaMET__sT", 25, -5, 5, 18, -3.1416,+3.1416 );
      CreateUserTH2D("h2_indexJet_vs_MaxDeltaMET__sT", 50, -250, 250, 15, 0, 15 );
      CreateUserTH1D("h_JetChargedEMFrac_LowNeutrPt__sT", 100, 0 , 1);
      CreateUserTH1D("h_JetChargedEMFrac_HighNeutrPt__sT", 100, 0 , 1);
      CreateUserTH1D("h_JetNeutralEMFrac_LowNeutrPt__sT", 100, 0 , 1);
      CreateUserTH1D("h_JetNeutralEMFrac_HighNeutrPt__sT", 100, 0 , 1);
      CreateUserTH1D("h_JetChargedHADFrac_LowNeutrPt__sT", 100, 0 , 1);
      CreateUserTH1D("h_JetChargedHADFrac_HighNeutrPt__sT", 100, 0 , 1);
      CreateUserTH1D("h_JetNeutralHADFrac_LowNeutrPt__sT", 100, 0 , 1);
      CreateUserTH1D("h_JetNeutralHADFrac_HighNeutrPt__sT", 100, 0 , 1);
      CreateUserTH2D("h_JetEtaPhiMap_LowNeutrPt__sT", 60, -3 , 3, 60, -3.1416,+3.1416);
      CreateUserTH2D("h_JetEtaPhiMap_HighNeutrPt__sT", 60, -3 , 3, 60, -3.1416,+3.1416);
      CreateUserTH1D("h_Njet__sT",  15, 0, 15 );
    }    
  ...
  // for (Long64_t jentry=0; jentry<1000;jentry++) { // Begin of loop over events
    ...
    std::auto_ptr<std::vector<double> >  JetChargedEMFrac  ( new std::vector<double>()  );
    std::auto_ptr<std::vector<double> >  JetChargedHADFrac  ( new std::vector<double>()  );
    std::auto_ptr<std::vector<double> >  JetNeutralEMFrac  ( new std::vector<double>()  );
    std::auto_ptr<std::vector<double> >  JetNeutralHADFrac  ( new std::vector<double>()  );
	...
	for (int ijet=0 ; ijet< PFJetPt->size() ; ijet++)
	  {
	    ...
	    JetChargedEMFrac->push_back( PFJetChargedEmEnergyFraction->at(ijet) );
	    JetChargedHADFrac->push_back( PFJetChargedHadronEnergyFraction->at(ijet) );
	    JetNeutralEMFrac->push_back( PFJetNeutralEmEnergyFraction->at(ijet) ); 
	    JetNeutralHADFrac->push_back( PFJetNeutralHadronEnergyFraction->at(ijet) );
...
//       } // printouts for jet cleaning
	...
	vector<int> v_idx_jet_PtCut_2;	    
	vector<int> v_idx_genjet_matched; //matched with v_idx_jet_PtCut_2	    
	...
	if(isData==0)
	  {	    
		...
		int idx_genjet_minDeltaR = -1;

		//pT pre-cut on reco jets
		if ( JetPt->at(ijet) < jet_PtCut_2 ) continue;

		jet.SetPtEtaPhiM(JetPt->at(ijet),
				 JetEta->at(ijet),
				 JetPhi->at(ijet),0);
		...
		for(int igenjet=0; igenjet<GenJetPt->size(); igenjet++)
		  {
		    ...
		    if(deltaR < minDeltaR)
		      {			
			minDeltaR = deltaR;
			idx_genjet_minDeltaR = igenjet;
		      }
		  }//end loop over gen jets	       

		if( minDeltaR < 0.1 )
		  {
		    v_idx_jet_PtCut_2.push_back(ijet);
		    v_idx_genjet_matched.push_back(idx_genjet_minDeltaR);
		  }
	...
	// The mismeasured jet most contributing to a change between reco MET and "true" MET
	double MaxDeltaMET = 0 ;
	int idx_MaxDeltaMET = -1 ;
	int pos_MaxDeltaMET = -1 ;
	double PtRecoOverPtGen_forMaxDeltaMET = -1;
	for(int ijet=0; ijet<v_idx_jet_PtCut_2.size(); ijet++)
	  {
	    double DeltaMET = 0;
	    TLorentzVector recojet, genjet;
	    TVector2 recojet_transv, genjet_transv, met;
	    recojet.SetPtEtaPhiM(JetPt->at(v_idx_jet_PtCut_2[ijet]),
				 JetEta->at(v_idx_jet_PtCut_2[ijet]),
				 JetPhi->at(v_idx_jet_PtCut_2[ijet]),0);		
	    genjet.SetPtEtaPhiM(GenJetPt->at(v_idx_genjet_matched[ijet]),
				GenJetEta->at(v_idx_genjet_matched[ijet]),
				GenJetPhi->at(v_idx_genjet_matched[ijet]),0);		    
	    recojet_transv.SetMagPhi(JetPt->at(v_idx_jet_PtCut_2[ijet]),
				     JetPhi->at(v_idx_jet_PtCut_2[ijet]) );
	    genjet_transv.SetMagPhi(GenJetPt->at(v_idx_genjet_matched[ijet]),
				    GenJetPhi->at(v_idx_genjet_matched[ijet]) );
	    met.SetMagPhi( thisMET , thisMETPhi );
	    
	    DeltaMET = met.Mod() - ( met - genjet_transv + recojet_transv ).Mod();

	    if(fabs(DeltaMET) > fabs(MaxDeltaMET) )
	      {
		MaxDeltaMET = DeltaMET;
		idx_MaxDeltaMET = v_idx_jet_PtCut_2[ijet];
		pos_MaxDeltaMET = ijet;
		PtRecoOverPtGen_forMaxDeltaMET = double( recojet.Pt() / genjet.Pt() );
	      }
	    
	    FillUserTH1D("h_PTrecoJet_Over_PTgenJetMatched", double( recojet.Pt() / genjet.Pt() ) );
	    FillUserTH1D("h_DeltaR_recoJet_genJetMatched", double( recojet.DeltaR(genjet) ) );
	    FillUserTH1D("h_DeltaMET", DeltaMET );
	    if( double( recojet.Pt() / genjet.Pt() )<0.6 )
	      FillUserTH2D("h2_EtaPhiMap_JetResponseLess0.6", recojet.Eta(), recojet.Phi() );

	  }

	// 	cout << "idx: " << idx_MaxDeltaMET << endl;
	// 	cout << "pos: " << pos_MaxDeltaMET << endl;
	// 	cout << "size: " << v_idx_jet_PtCut_2.size() << endl;
	// 	if( idx_MaxDeltaMET==-1 && pos_MaxDeltaMET==-1 && v_idx_jet_PtCut_2.size()==0)
	// 	  {
	// 	    cout << "recojets" << endl;
	// 	    for(int ijet=0; ijet<JetPt->size(); ijet++)
	// 	      {
	// 		cout << JetPt->at(ijet) << " " << JetEta->at(ijet) << " " << JetPhi->at(ijet) << endl;
	// 	      }
	// 	    cout << "genjets" << endl;
	// 	    for(int ijet=0; ijet<GenJetPt->size(); ijet++)
	// 	      {
	// 		cout << GenJetPt->at(ijet) << " " << GenJetEta->at(ijet) << " " << GenJetPhi->at(ijet) << endl;
	// 	      }
	// 	  }
	
 	if( idx_MaxDeltaMET!=-1 && pos_MaxDeltaMET!=-1)
 	  {
	    FillUserTH1D("h_MaxDeltaMET", MaxDeltaMET );
	    FillUserTH2D("h2_EtaPhiMap_JetWithLargestDeltaMET", JetEta->at(idx_MaxDeltaMET), JetPhi->at(idx_MaxDeltaMET) );
	    FillUserTH2D("h2_indexJet_vs_MaxDeltaMET", MaxDeltaMET, pos_MaxDeltaMET );
	    if(MaxDeltaMET>0)
	      FillUserTH1D("h_PTrecoJet_Over_PTgenJetMatched_PosMaxDeltaMET", PtRecoOverPtGen_forMaxDeltaMET );
	    else
	      FillUserTH1D("h_PTrecoJet_Over_PTgenJetMatched_NegMaxDeltaMET", PtRecoOverPtGen_forMaxDeltaMET );
	  }
	//------------------------------------------------------
	...
	if( passedAllPreviousCuts("sT_MLQ200") && passedCut("sT_MLQ200")
	    ...
	    )
	  {
	    ...
	    if( idx_MaxDeltaMET!=-1 && pos_MaxDeltaMET!=-1)
	      {		
		FillUserTH1D("h_MaxDeltaMET__sT", MaxDeltaMET );
		FillUserTH2D("h2_EtaPhiMap_JetWithLargestDeltaMET__sT", JetEta->at(idx_MaxDeltaMET), JetPhi->at(idx_MaxDeltaMET) );
		FillUserTH2D("h2_indexJet_vs_MaxDeltaMET__sT", MaxDeltaMET, pos_MaxDeltaMET );
	      }

	    //jets in fake MET and real MET events	    
	  
	    FillUserTH1D("h_Njet__sT", v_idx_jet_PtCut_noOverlap_ID.size() );

	    if( NeutrinoSum.Pt() < 50 )
	      {
		for(int ijet=0; ijet<v_idx_jet_PtCut_noOverlap_ID.size(); ijet++)
		  {
		    FillUserTH1D("h_JetChargedEMFrac_LowNeutrPt__sT", JetChargedEMFrac->at(v_idx_jet_PtCut_noOverlap_ID[ijet]) );
		    FillUserTH1D("h_JetNeutralEMFrac_LowNeutrPt__sT", JetNeutralEMFrac->at(v_idx_jet_PtCut_noOverlap_ID[ijet]) );
		    FillUserTH1D("h_JetChargedHADFrac_LowNeutrPt__sT", JetChargedHADFrac->at(v_idx_jet_PtCut_noOverlap_ID[ijet]) );
		    FillUserTH1D("h_JetNeutralHADFrac_LowNeutrPt__sT", JetNeutralHADFrac->at(v_idx_jet_PtCut_noOverlap_ID[ijet]) );
		    FillUserTH2D("h_JetEtaPhiMap_LowNeutrPt__sT", JetEta->at(v_idx_jet_PtCut_noOverlap_ID[ijet]), JetPhi->at(v_idx_jet_PtCut_noOverlap_ID[ijet]) );
		  }
	      }
	    else
	      {
		for(int ijet=0; ijet<v_idx_jet_PtCut_noOverlap_ID.size(); ijet++)
		  {
		    FillUserTH1D("h_JetChargedEMFrac_HighNeutrPt__sT", JetChargedEMFrac->at(v_idx_jet_PtCut_noOverlap_ID[ijet]) );
		    FillUserTH1D("h_JetNeutralEMFrac_HighNeutrPt__sT", JetNeutralEMFrac->at(v_idx_jet_PtCut_noOverlap_ID[ijet]) );
		    FillUserTH1D("h_JetChargedHADFrac_HighNeutrPt__sT", JetChargedHADFrac->at(v_idx_jet_PtCut_noOverlap_ID[ijet]) );
		    FillUserTH1D("h_JetNeutralHADFrac_HighNeutrPt__sT", JetNeutralHADFrac->at(v_idx_jet_PtCut_noOverlap_ID[ijet]) );
		    FillUserTH2D("h_JetEtaPhiMap_HighNeutrPt__sT", JetEta->at(v_idx_jet_PtCut_noOverlap_ID[ijet]), JetPhi->at(v_idx_jet_PtCut_noOverlap_ID[ijet]) );
		  }
	      }
	  
	  }//end pass full selection
