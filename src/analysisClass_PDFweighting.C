...
void analysisClass::Loop()
{
  ...
  TH1F *h_PDFWeight_Npass_400 = new TH1F ("PDFWeight_Npass_400","PDFWeight_Npass_400",41,-0.5,40.5); h_PDFWeight_Npass_400->Sumw2();

  double N_events[41];

  double cum_weights_PASS_200[41];
  double cum_weights_PASS_300[41];
  double cum_weights_PASS_400[41];
  for (int i=0;i<41;i++){
    cum_weights_PASS_200[i]=0;
    cum_weights_PASS_300[i]=0;
    cum_weights_PASS_400[i]=0;
    N_events[i]=0;
  }
...
// 	fillVariableWithValue("minDR_2ele_2jet", minDR_2ele_2jet);
    ...
    if( passedAllPreviousCuts("sT_MLQ250") ){
      double CentralWeight = PDFWeights->at(0) ;
      for(int pdf=0;pdf<PDFWeights->size();pdf++){
	double tmp = (PDFWeights->at(pdf)/CentralWeight);
	if (tmp!=tmp) continue; // solves nan problem
    ...
    if( passedAllPreviousCuts("sT_MLQ400") ){
      double CentralWeight = PDFWeights->at(0) ;
      for(int pdf=0;pdf<PDFWeights->size();pdf++){
	double tmp = (PDFWeights->at(pdf)/CentralWeight);
	if (tmp!=tmp) continue; // solves nan problem
